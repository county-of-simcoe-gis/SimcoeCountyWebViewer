...
class SearchMTO extends Component {
  constructor(props) {
    super(props);
  ...
  onHistoryItemSelect = (item) => {
    ...
    this.setState({ value: value, searchResults: searchResults }, () => {
      this.onItemSelect(item.value, item);
    });
  };
  onMapLoad = () => {
    ...
    if (result.layers !== undefined && result.layers !== null) {
      ...
      if (assocLayers.length > 0) {
        Object.entries(window.allLayers).forEach((row) => {
          ...
          const currentLayers = layerItems.filter((layer) => {
            return assocLayers.includes(layer.name.toLowerCase());
          });
