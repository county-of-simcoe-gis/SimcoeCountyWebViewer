...
class LoadingScreen extends Component {
    ...
    componentDidMount() {
        this.setState({isVisible:this.props.visible},()=>{
            this.getStyles(styles => {
                this.setState({ spinnerStyle: styles.spinnerStyle, messageStyle: styles.messageStyle, containerStyle:styles.containerStyle});
            });
        });
    }
    //CONVERT hex to rgb
    ...
    getStyles = callback => {
        ...
        if (this.state === undefined || !this.state.isVisible) callback({containerStyle:{ display: "none" },spinnerStyle:{ display: "none" },messageStyle:{ display: "none" }});
            let spinnerSize = 120;
    ...
    render() {
        return (
            <div className={this.props.visible?"sc-loading-screen":"sc-hidden"} style={this.state.containerStyle}>
                <div className="spinner" style={this.state.spinnerStyle} ></div>
                <div className="message" style={this.state.messageStyle} >{this.props.message}</div>
            </div>
        );
      }
}
