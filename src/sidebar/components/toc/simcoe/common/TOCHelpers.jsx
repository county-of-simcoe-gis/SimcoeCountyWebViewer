...
// GET GROUPS FROM GET CAPABILITIES
export async function getGroupsGC(url, urlType, isReset, tocType, callback) {
  let defaultGroup = null;
  let isDefault = false;
  let groups = [];
            ...
            const buildLayers = (layers) => {
              layers.forEach((currentLayer) => {
                if (!isDuplicate(layerList, currentLayer.Name[0])) {
                  buildLayerByGroup(tmpGroupObj, currentLayer, layerIndex, tocType, (result) => {
                    layerList.push(result);
                  });
                  layerIndex--;
...
export async function buildLayerByGroup(group, layer, layerIndex, tocType, callback) {
  // SAVED DATA
  ...
  if (layer.Layer === undefined) {
    ...
    // LAYER PROPS
    LayerHelpers.getLayer(OL_DATA_TYPES.ImageWMS, "WMS", undefined, layer.Name[0], serverUrl + "/wms?layers=" + layer.Name[0], false, undefined, undefined, displayName, (newLayer) => {
    ...
    parser.parseString(result, function(err, result) {
      ...
      groupLayerList.forEach((layerInfo) => {
        if (!isDuplicate(layerList, layerInfo.Name[0])) {
          if (layerInfo.Layer === undefined) {
            buildLayerByGroup(group, layerInfo, layerIndex, tocType, (result) => {
              layerList.push(result);
            });
            layerIndex--;
          }
    ...
    groupLayerList.forEach((layerInfo) => {
      ...
      if (!isDuplicate(layerList, layerNameOnly)) {
        ...
        const wfsUrlTemplate = (serverUrl, layerName) =>
          `${serverUrl}/wfs?service=wfs&version=2.0.0&request=GetFeature&typeNames=${layerName.split(" ").join("%20")}&outputFormat=application/json&cql_filter=`;
