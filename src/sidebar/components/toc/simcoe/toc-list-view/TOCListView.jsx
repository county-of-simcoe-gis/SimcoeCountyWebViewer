...
class TOCListView extends Component {
  ...
  onActivateLayer = (groupName, callback) => {
    const tocFolder = document.getElementById("sc-toc-simcoe-list-view-container-main");
  ...
  refreshTOC = (isReset, callback) => {
    ...
    if (geoserverUrl !== undefined && geoserverUrl !== null) {
      TOCHelpers.getGroupsGC(geoserverUrl, geoserverUrlType, isReset, "LIST", (result) => {
    ...
    const menu = (
      <Portal>
        <FloatingMenu key={helpers.getUID()} buttonEvent={evtClone} item={this.props.info} onMenuItemClick={(action) => this.onMenuItemClick(action)} styleMode="right" yOffset={90}>
          <MenuItem className="sc-floating-menu-toolbox-menu-item" key="sc-floating-menu-switch">
            <FloatingMenuItem imageName={"collapsed.png"} label="Switch to List View" />
          </MenuItem>
  ...
  onMenuItemClick = (action) => {
    switch (action) {
      ...
      case "sc-floating-menu-switch":
        this.props.onTypeChange("FOLDER");
        break;
      ...
      case "sc-floating-menu-reset":
        this.reset();
        helpers.addAppStat("TOC Reset", "Click");
        break;
      default:
        break;
    }
