				...
				(layerGroupConfig) => {
					...
					if (sourcesProcessed === mapSettings.sources.length) {
						if (mapSettings.default_group !== undefined) {
							const mapDefaultGroup = layerGroups.filter((group) => {
								return group.value === mapSettings.default_group;
							})[0];
							if (mapDefaultGroup !== undefined) defaultGroup = mapDefaultGroup;
							else
								helpers.showMessage(
									`Configuration Error!`,
									`Default group ${mapSettings.default_group} could not be loaded.`,
									helpers.messageColors.red,
									3000
								);
						}
						if (defaultGroup === undefined || defaultGroup === null)
							defaultGroup = layerGroups[0];
...
export function getBase64FromImageUrlWithParams(
	url,
	params = undefined,
	callback
) {
...
export async function buildLayerByGroup(
	...
	if (layer.Layer === undefined) {
		...
		// LAYER PROPS
		LayerHelpers.getLayer(
			OL_DATA_TYPES.ImageWMS,
			"WMS",
			undefined,
			layer.Name,
			serverUrl + "/wms?layers=" + layer.Name,
			false,
			undefined,
			undefined,
			displayName,
