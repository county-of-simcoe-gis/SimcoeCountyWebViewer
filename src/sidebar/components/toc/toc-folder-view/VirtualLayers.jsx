...
// LIST ITEM SORTABLE
const SortableItem = sortableElement(({ value, style, item, searchText, virtualId, onLayerChange, group }) => {
  ...
  return (
    <li key={helpers.getUID()} style={style} className="sc-toc-layer-list-item sc-noselect" id={item.elementId}>
      <LayerItem key={helpers.getUID()} layer={item} virtualId={virtualId} onLayerChange={onLayerChange} searchText={searchText} group={group} />
    </li>
  );
});
