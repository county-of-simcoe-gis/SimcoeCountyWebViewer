...
const Forecast = (props) => {
	...
	const iconUrl = (code) =>
		`https://weather.gc.ca/weathericons/small/${code}.png`;
	...
	return (
		<div className="sc-tool-weather-forecast-day">
			<label className="sc-tool-weather-forecast-dt">
				{siteData.forecastGroup[0].dateTime[1].textSummary[0]}
			</label>
			<div>
				<fieldset className="sc-fieldset" style={{ marginTop: "0px" }}>
					<legend className="sc-tool-weather-forecast-legend">
						{siteData.location[0].name[0]._}
					</legend>
					<div
						className={
							warningEvents.length === 0
								? "sc-hidden"
								: "sc-tool-weather-forecast-warnings-container"
						}
					>
					...
					<div
						className={
							warningEvents.length === 2
								? "sc-tool-weather-forecast-days-container two-warnings"
								: "sc-tool-weather-forecast-days-container"
						}
					>
						<div
							className="sc-tool-weather-forecast-day-container"
							title={forecast1.textSummary[0]}
						>
							<div className="sc-tool-weather-forecast-day-details">
								<label>{forecast1.period[0].$.textForecastName}</label>
								<img
									className="sc-tool-weather-forecast-details-img"
									src={iconUrl(forecast1.abbreviatedForecast[0].iconCode[0]._)}
									alt="icon"
								/>
								<label>{forecast1.temperatures[0].temperature[0]._}</label>
								<label className="sc-tool-weather-tool-forecast-summary">
									{forecast1.abbreviatedForecast[0].textSummary[0]}
								</label>
							</div>
						</div>
						<div
							className="sc-tool-weather-forecast-day-container"
							title={forecast2.textSummary[0]}
						>
							<div className="sc-tool-weather-forecast-day-details">
								<label>{forecast2.period[0].$.textForecastName}</label>
								<img
									className="sc-tool-weather-forecast-details-img"
									src={iconUrl(forecast2.abbreviatedForecast[0].iconCode[0]._)}
									alt="icon"
								/>
						...
						<div
							className="sc-tool-weather-forecast-day-container"
							title={forecast3.textSummary[0]}
						>
							<div className="sc-tool-weather-forecast-day-details">
								<label>{forecast3.period[0].$.textForecastName}</label>
								<img
									className="sc-tool-weather-forecast-details-img"
									src={iconUrl(forecast3.abbreviatedForecast[0].iconCode[0]._)}
									alt="icon"
								/>
...
const Warning = (props) => {
	...
	return (
		<div
			className={
				type === "ENDED"
					? "sc-tool-weather-forecast-warning ended"
					: "sc-tool-weather-forecast-warning " + priority
			}
		>
