...
let computeExtent = (height, width, dpi, scale, center) => {
  const xCenter = center[0];
  const yCenter = center[1];

  // dividing image width by DPI to get it in Inch then convert to meters
  let imgWidthInMeter = (width / dpi) * 0.0254;
  let imgHeightInMeter = (height / dpi) * 0.0254;

  // calculating half of map’s height & width at the specific scale
  let dX = (imgWidthInMeter * scale) / 2;
  let dY = (imgHeightInMeter * scale) / 2;
  let minX = xCenter - dX;
  let maxX = xCenter + dX;
  let minY = yCenter - dY;
  let maxY = yCenter + dY;
