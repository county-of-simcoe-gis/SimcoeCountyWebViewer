...
class ThemeDataList extends Component {
  ...
  componentDidMount(){
    ...
    // GET LEGEND
    const styleUrlTemplate = (serverURL, layerName, styleName) => `${serverURL}/wms?REQUEST=GetLegendGraphic&VERSION=1.1&FORMAT=image/png&WIDTH=30&HEIGHT=30&LAYER=${layerName}&STYLE=${styleName}&transparent=true`;
  ...
  fetchFeatures = () => {
    ...
    if (this.state.onlyFeaturesWithinMap){
      ...
      }, this.props.layerConfig.displayFieldName, extent );
    } else {
      helpers.getWFSGeoJSON(this.props.layerConfig.serverUrl,this.props.layerConfig.layerName, (result) => {
      this.setState({features: result});
  ...
  itemClick = (feature) => {
    ...
    if (geoType !== "Point")
      center = turf.centroid(feature.getGeometry());
