...
class ThemeLayerToggler extends Component {
  ...
  handleUrlParameter = () => {
    if (this.props.layerConfig.UrlParameter === undefined)
      return;
    ...
    helpers.getWFSGeoJSON(this.props.layerConfig.serverUrl,this.props.layerConfig.layerName, (result) => {
      if (result.length === 0)
        return;
      ...
      window.popup.show(center, <ThemePopupContent key={helpers.getUID()} values={entries} popupLogoImage={this.props.mainConfig.popupLogoImage}
      layerConfig={this.props.layerConfig}></ThemePopupContent>,  this.props.layerConfig.displayName);
    }, null, null, query );
  ...
  componentDidMount(){
    ...
    // GET LEGEND
    const styleUrlTemplate = (serverURL, layerName, styleName) => `${serverURL}/wms?REQUEST=GetLegendGraphic&VERSION=1.1&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=${layerName}&STYLE=${styleName}`;
    ...
    this.mapClickEvent = window.map.on('click', (evt) => {
      ...
      var url = this.state.layer.getSource().getGetFeatureInfoUrl(
      ...
      if (url) {
        helpers.getJSON(url, (result) => {
          ...
          </ThemePopupContent>,  this.props.layerConfig.displayName);
        })
      }
    });
    ...
    return ( 
      <div className="sc-theme-layer-container">
        <div className="sc-theme-layer-toggler-symbol">
          <img src={this.state.styleUrl} alt="style"></img>
        </div>
        ...
        <label className="sc-theme-layer-toggler-count">{" (" + this.state.recordCount + ")"}</label>
        <div>
        {this.props.children}
        </div>
      </div>
