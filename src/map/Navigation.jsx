...
class Navigation extends Component {
    ...
    zoomIn() {
        ...
        console.log(window.map.getView().getZoom());
        console.log(window.map.getView().getResolution());
    }
    ...
    zoomFullExtent(){
        if (window.sidebarOpen)
            window.map.getView().animate({center: [-8875141.45, 5543492.45], zoom: 10});
        else
            window.map.getView().animate({center:  [-8878504.68, 5543492.45], zoom: 10});
    }
    ...
    render() { 
        ...
        <div className={this.state.containerClassName}>
            <div className="zoomButton" onClick={this.zoomIn}>+</div>
            <div className="zoomButton" onClick={this.zoomOut}>-</div>
